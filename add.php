  ...
  if (!empty($_FILES['image']['name'])) {
    ...
    if ($file['error'] === UPLOAD_ERR_OK) {
      ...
      if (!isset($allowed[$type])) {
        $errors[] = 'Please upload an image (jpg, png, gif, webp, svg).';
      } elseif ($file['size'] > 2 * 1024 * 1024) {
        $errors[] = 'Image must be under 2MB.';
      } else {
        $ext = $allowed[$type];
        $imageName = time() . '_' . preg_replace('/[^a-z0-9\-_]+/i','-', pathinfo($file['name'], PATHINFO_FILENAME)) . '.' . $ext;
        $dest = __DIR__ . '/assets/images/skills/' . $imageName;
        if (!move_uploaded_file($file['tmp_name'], $dest)) {
          $errors[] = 'Could not save uploaded file.';
          $imageName = null;
        }
        // When deploying to Coreteaching, ensure skills folder permissions 777.
      }
    } else {
...
<form id="skillForm" class="form" action="" method="post" enctype="multipart/form-data" novalidate>
  ...
  <label>Category *<input type="text" name="category" placeholder="e.g., Music, Art, Cooking" required value="<?= htmlspecialchars($_POST['category'] ?? '') ?>"></label>
  ...
  <label>Skill Image
    <input type="file" name="image" id="image" accept=".jpg,.jpeg,.png,.gif,.webp,.svg" />
  </label>
